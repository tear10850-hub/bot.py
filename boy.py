...
# Render Free Tier မရပ်သွားစေရန် Flask Server
app_flask = Flask('')
...
OWNER_ID = 7771663458  
CHANNEL_LINK = "https://t.me/BOTUAPTE"  
GROUP_ADD_LINK = "https://t.me/Cupi677Bot?startgroup=true"  
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.chat.type == "private":
        ...
        caption = f"သုံးဆွဲသူရဲ့အချက်အလက်ရှင့်\nအမည်🤔- {name}\nId🌷: {user_id}\n@🌸: {username}\nBio🙂: {bio}\n\nBy @Cupi677Bot 🤭🤭"
        ...
        await message.reply_text("🤖 မင်္ဂလာပါရှင့်! @Cupi677Bot အသင့်ဖြစ်ပါပြီရှင့် 🌸။")

# Hugging Face API ဖြင့် ချိတ်ဆက်၍ AI ဖြေကြားပေးမည့် စနစ်
async def ai_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if AI_API_KEY:
        try:
            ...
            API_URL = "https://api-inference.huggingface.co/models/facebook/blenderbot-400M-extract"
            ...
            if response.status_code == 200:
                ...
                if isinstance(res_json, list) and len(res_json) > 0:
                    ai_reply_text = res_json[0].get("generated_text", "")
                elif isinstance(res_json, dict):
                    ai_reply_text = res_json.get("generated_text", "")
        except Exception as e:
            logger.error(f"Hugging Face API Error: {e}")

    # API မှ အဖြေမရပါက သို့မဟုတ် အရန်အနေဖြင့် ချိုသာသော မိန်းကလေးလေသံဖြင့် ပေါင်းစပ်ဖြေကြားရန်
    if not ai_reply_text:
        ai_reply_text = f"ဟင်... ကိုကိုမေးတဲ့ '{query}' ဆိုတာလေးကို တွေးနေမိလို့ပါရှင့်။ ကိုကို့အတွက် အမြဲတမ်း အသင့်ရှိနေပါတယ်နော် 🤭💕✨"
    else:
        ai_reply_text = f"ဟင့်အင်း... ရှင့်မေးခွန်းကို ဖြေရရင် - {ai_reply_text} 🌸✨"

    await message.reply_text(ai_reply_text, reply_to_message_id=message.message_id)
...
async def handle_group_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Sticker အသစ်များ Owner ထံ ခွင့်တောင်းခံရန်
        if message.sticker:
            if is_owner:
                try:
        ...
        # Group ထဲတွင် ရံဖန်ရံခါ အလိုအလျောက် စကားပြန်ပေးမည့်စနစ်
