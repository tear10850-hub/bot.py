...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = "🤖 မင်္ဂလာပါ! ကျွန်တော် Bot စတင် အလုပ်လုပ်နေပါပြီခင်ဗျာ။ စာဝင်လာတာနဲ့ တစ်ကြောင်းချင်းစီ အမြန်ဆုံး ပြန်ပြောပေးသွားပါမယ်။"
    await update.message.reply_text(welcome_text)

# 💬 Group ထဲက မက်ဆေ့ဂျ်များကို ကိုင်တွယ်မည့် Function (အငြိမ်ဆုံးနှင့် ချက်ချင်း Reply ပြန်မည့်ပုံစံ)
async def handle_group_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if message.text:
            ...
            # MongoDB ထဲရှိ စာအားလုံးကို တစ်ခါတည်း ဆွဲထုတ်ခြင်း (Cache/List)
            all_messages = [doc["user_text"] for doc in collection.find({}, {"_id": 0, "user_text": 1})]

            if all_messages:
                # သိမ်းထားသမျှ စာတွေထဲက တစ်ခုကို ကျပန်းရွေးရန်
                ...
                # ဝင်လာတဲ့ စာတိုင်းအတွက် Reply အသေအချာ ပြန်ပေးရန်
...
def main():
    ...
    logger.info("🤖 Bot သည် တည်ငြိမ်စွာ နှင့် အမြန်ဆုံး အလုပ်လုပ်နေပါပြီ...")
    app.run_polling()
