...
try:
    ...
    client.admin.command('ping') # Connection ကို စစ်ဆေးခြင်း
...
async def ask_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # မြန်မာဘာသာဖြင့် သဘာဝကျကျ ဖြေကြားစေရန် ညွှန်ကြားချက်
    ...
    try:
        # Timeout (၁၅ စက္ကန့်) ထည့်ထားခြင်းဖြင့် ဆာဗာချို့ယွင်းမှုကို ကာကွယ်မည်
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # 1. မက်ဆေ့ခ်ျ သိမ်းဆည်းခြင်း Logic
        ...
        # 2. Group ထဲတွင် (သို့မဟုတ် Chat ထဲတွင်) အခြားသူများ ပို့လာပါက သိမ်းထားသည်များထဲမှ ကျပန်းပြန်မည်
        if chat.type in ["group", "supergroup"]:
            await context.bot.send_chat_action(chat_id=chat.id, action="typing")
            
            # Database ထဲမှ အချက်အလက်များကို လုံခြုံစွာ ထုတ်ယူခြင်း
...
def main():
    ...
    application.add_handler(MessageHandler(filters.TEXT | filters.Sticker, handle_message))
