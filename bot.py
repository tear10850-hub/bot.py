...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.chat.type == "private":
        ...
        caption = f"သူဆွဲသူရဲ့အချက်အလက်ရှင့်\nအမည်🤔- {name}\nId🌷: {user_id}\n@🌸: {username}\nBio🙂: {bio}\n\nBy @Cupi677Bot 🤭🤭"
...
async def handle_group_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Sticker စနစ် (Member တွေပို့ရင် ကျော်မယ်၊ Owner ပို့ရင် သိမ်းမယ်)
        if message.sticker:
            if is_owner:
                try:
                ...
                return  # Member တွေပို့တဲ့ Sticker ဆိုရင် Bot က လုံးဝကျော်သွားပါမယ်
        ...
        # Group ထဲတွင် ရံဖန်ရံခါ အလိုအလျောက် MongoDB ထဲက စာ/စတစ်ကာ ဖြင့် Reply (ပြန်စာ) ပြန်ပေးမည့်စနစ်
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("sticker_yes_"):
        ...
        try:
            ...
            await query.edit_message_text("✅ ဤ Sticker ကို Database ထဲသို့ အောင်မြင်စွာ သိမ်းဆည်းလိုက်ပါရှင့်။")
        except Exception:
