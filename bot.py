...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.chat.type == "private":
        ...
        caption = f"သုံးဆွဲသူရဲ့အချက်အလက်\nအမည်🤔- {name}\nId🌷: {user_id}\n@🌸: {username}\nBio🙂: {bio}\n\nBy mya\"🤭🤭"
...
async def track_group_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.new_chat_members:
        for user in message.new_chat_members:
            ...
            # Group ထဲဝင်လာတဲ့သူရဲ့ Bio ကို ဆွဲထုတ်ရန် ကြိုးစားခြင်း
            bio = "မရှိပါ"
            try:
                chat_full = await context.bot.get_chat(user_id)
                if chat_full and chat_full.bio:
...
async def handle_group_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if chat.type == "private":
            await message.reply_text(f"🤖 မင်္ဂလာပါရှင့်! AI မှ ပြန်ကြားချက် - '{message.text}' ဆိုတာကို လက်ခံရရှိပါပြီ။")
            return
