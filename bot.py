...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if chat and user:
            ...
            # Bot အောက်ခြေတွင် အမြဲပေါ်နေမည့် Reply Keyboards များ (အောက်ဆုံးတွင် ရှိမည်)
            reply_keyboard = [
            ...
            if saved_video and "file_id" in saved_video:
                ...
                # အောက်ခြေခလုတ်များကိုပါ တစ်ပါတည်း တင်ပေးလိုက်သည်
...
async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ အမြင်ကပ်ပုတ်မှနှင့်😏{reply.from_user.full_name} ကို Group မှ Ban လိုက်ပါပြီ။")
    except Exception as e:
...
async def handle_chat_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if message.left_chat_member:
            ...
            if left_user.id == context.bot.id: continue
