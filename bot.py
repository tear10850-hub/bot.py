...
# အဓိကသုံးမည့် AI Model (Llama 3 8B)
AI_MODEL_URL = "https://router.huggingface.co/hf-inference/models/meta-llama/Meta-Llama-3-8B-Instruct"
...
# 🎬 4. Video အသစ် သတ်မှတ်ခြင်း (/startvideo နှင့် /Tvideo)
...
# 🤖 5. AI COMMAND HANDLER (/ai နှင့် Reply ထောက်၍ မေးမြန်းခြင်း)
async def ask_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if reply_msg and reply_msg.text:
        user_message = f"{reply_msg.text} {user_message}".strip()

    if not user_message and update.message.text:
        clean_text = update.message.text.replace("/ai", "").strip()
        if clean_text:
            user_message = clean_text

    if not user_message:
        await update.message.reply_text("ကျေးဇူးပြု၍ မေးလိုသည့် မေးခွန်းကို ရေးပေးပါ သို့မဟုတ် စာကို Reply ထောက်ပြီး မေးပါ။")
        return
    ...
    payload = {
        "inputs": f"Answer this in Burmese naturally with full details: {user_message}",
        "parameters": {"max_new_tokens": 400, "return_full_text": False}
    }

    try:
        response = requests.post(AI_MODEL_URL, headers=headers, json=payload, timeout=30)
...
# 👋 7. Group ထဲသို့ လူအသစ်ဝင်လာခြင်း နှင့် ထွက်သွားခြင်းကို ကိုင်တွယ်ရန်
        ...
        if message.new_chat_members:
            for new_user in message.new_chat_members:
                ...
                caption_text = (
                    ...
                    f"🆔 ID: `{user_id}`\n"
                    f"Myaရှိတဲ Group လေးသို့ ဖိတ်ခေါ်ပါတယ်ရှင့်လူလေးထည့်ပေးခဲ့အုန်း😜။"
                )
...
# 💬 8. Group/Chat ထဲက မက်ဆေ့ဂျ်များကို ကိုင်တွယ်ခြင်း (Owner ပို့သော စတစ်ကာများကိုသာ မှတ်သားမည်)
async def handle_group_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # စတစ်ကာ စစ်ဆေးခြင်း (Owner ပို့မှသာ Database ထဲ သိမ်းမည်၊ ကျန်သူများပို့လျှင် ကျော်သွားမည်)
        if message.sticker:
            emoji = message.sticker.emoji if message.sticker.emoji else ""
            if any(e in emoji for e in BLOCKED_EMOJIS):
                return
            
            # OWNER_ID ဖြစ်မှသာ စတစ်ကာကို Database ထဲသို့ မှတ်သားမည်
...
def main():
    ...
    logger.info("🤖 Bot သည် Render Web Service ပေါ်တွင် Port ဖွင့်၍ အောင်မြင်စွာ အလုပ်လုပ်နေပါပြီ...")
    app.run_polling()
