...
CHANNEL_LINK = "https://t.me/your_channel"  # ⚠️ သင့် Channel Link ထည့်ရန်
GROUP_ADD_LINK = "https://t.me/your_bot?startgroup=true"  # ⚠️ Bot ကို Group ထည့်ရန် Link

BLOCKED_EMOJIS = ["🍆", "🍑", "💦", "🔞"]

# Hugging Face NSFW Detection API
...
async def track_group_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.new_chat_members:
        for user in message.new_chat_members:
            ...
            keyboard = [[InlineKeyboardButton("🛠 Admin ကြည့်ရန် 👑", callback_data=f"show_admins_{chat.id}_0")]]
            reply_markup = InlineKeyboardMarkup(keyboard)

            setting = settings_col.find_one({"key": f"welcome_{chat.id}"})
            if setting and "file_id" in setting:
                try:
                    await context.bot.send_video(chat_id=chat.id, video=setting["file_id"], caption=caption, reply_markup=reply_markup)
                    continue
                except Exception:
                    pass
            
            await context.bot.send_message(chat_id=chat.id, text=caption, reply_markup=reply_markup)
    ...
    if message.left_chat_member:
        ...
        # 👤 ထွက်သွားတဲ့သူကို အီမုတ်/အမည်နဲ့ တွဲပြီး နှိပ်ရင် Profile ရောက်မယ့် Markdown Mention Link ဖန်တီးခြင်း
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("show_admins_"):
        ...
        try:
            ...
            if not human_admins:
                await query.answer("Admin မရှိပါ။", show_alert=True)
                return

            if index >= len(human_admins):
                index = 0
            elif index < 0:
                index = len(human_admins) - 1

            admin_member = human_admins[index]
            admin_user = admin_member.user

            name = admin_user.first_name if admin_user else "Admin"
            admin_id = admin_user.id
            username = f"@{admin_user.username}" if admin_user and admin_user.username else "မရှိပါ"

            # 🏷️ Admin ကို အီမုတ်လေးတွေနဲ့ အမည်တန်းခေါ်ပေးမည့် Mention Link (Click နှိပ်ရင် Profile ရောက်မည်)
            admin_mention = f"[{name} 👑](tg://user?id={admin_id})"

            bio = "မရှိပါ"
            try:
                chat_full = await context.bot.get_chat(admin_id)
                if chat_full.bio:
                    bio = chat_full.bio
            except Exception:
                pass
            ...
            admin_text = (
                f"👑 **Group Admin အချက်အလက်** ({index + 1}/{len(human_admins)})\n\n"
                f"ခေါ်ဆိုရန် 🤔- {admin_mention}\n"
                f"အမည် 📌- {name}\n"
                f"Id 🌷- {admin_id}\n"
                f"@ 🌸- {username}\n"
                f"Bio 🙂- {bio}\n\n"
                f"✨ *အောက်ပါ ခလုတ်ကို နှိပ်၍လည်း ဤ Admin ထံ တိုက်ရိုက်ဆက်သွယ်နိုင်ပါသည်ရှင့် 👇*"
            )

            photos = await context.bot.get_user_profile_photos(admin_id, limit=1)
            
            keyboard = [
                [InlineKeyboardButton(f"💌 ဆက်သွယ်ရန် ➡️ {name}", url=f"tg://user?id={admin_id}")],
                [
                    InlineKeyboardButton("⬅️ Back", callback_data=f"back_welcome_{chat_id}"),
                    InlineKeyboardButton("Next ➡️", callback_data=f"show_admins_{chat_id}_{index + 1}")
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            ...
            if photos.total_count > 0:
                ...
                try:
                    await query.message.edit_media(
                        media=InputMediaPhoto(media=photo_file_id, caption=admin_text, parse_mode="Markdown"),
                        reply_markup=reply_markup
                    )
                    return
                except Exception:
            ...
            try:
                await query.message.edit_text(text=admin_text, reply_markup=reply_markup, parse_mode="Markdown")
            except Exception:
                pass

        except Exception as e:
            logger.error(f"Admin များကို ဆွဲထုတ်ရာတွင် Error: {e}")
            await query.answer("Admin စာရင်း ထုတ်ယူ၍ မရပါ။", show_alert=True)

    elif data.startswith("back_welcome_"):
        try:
